...
class RelevantSkaterData:
    def __init__(self, raw_data):
        self.name = raw_data['name']
        self.pos = raw_data['position']
        self.team = raw_data['team']

        self.assists = float(raw_data['I_F_primaryAssists']) + float(raw_data['I_F_secondaryAssists'])
        self.expected_goals = float(raw_data['I_F_xGoals'])
        self.actual_goals = float(raw_data['I_F_goals'])
        self.attempted_shots = float(raw_data['I_F_shotAttempts'])
        self.actual_shots = float(raw_data['I_F_shotsOnGoal'])
        self.blocks = float(raw_data['shotsBlockedByPlayer'])
        self.hits = float(raw_data['I_F_hits'])
        self.ice_time = float(raw_data['icetime'])
        self.games_played = float(raw_data['games_played'])

        self.expected_value = self.get_expected_value()
        self.expected_value_pg = self.expected_value / self.games_played

    def get_expected_value(self) -> float:
        return (
            self.assists * 4.0 +
            self.actual_goals * 5.0 +
            self.blocks * 0.5 +
            self.hits * 0.5 +
            self.actual_shots * 0.1
        )

class AllSkaters:
    def __init__(self, csv_file_in: str):
        self.skaters: list[RelevantSkaterData] = []
        with open(csv_file_in, 'r') as csv_file:
            for skater in csv.DictReader(csv_file):
                if skater['situation'] != 'all':
                    continue
                if float(skater['games_played']) < 15.0:
                    continue
                self.skaters.append(RelevantSkaterData(skater))
        self.skaters.sort(key=lambda s: s.expected_value_pg, reverse=True)

    def ranked_defense(self) -> list[RelevantSkaterData]:
        return [s for s in self.skaters if s.pos == 'D']

    def ranked_centers(self) -> list[RelevantSkaterData]:
        return [s for s in self.skaters if s.pos == 'C']

    def ranked_lefties(self) -> list[RelevantSkaterData]:
        return [s for s in self.skaters if s.pos == 'L']

    def ranked_righties(self) -> list[RelevantSkaterData]:
        return [s for s in self.skaters if s.pos == 'R']
